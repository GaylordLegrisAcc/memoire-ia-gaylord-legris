...
L'évaluation de la déformation du bilan face à l'ensemble de ces chocs requiert une projection de long terme sur un grand nombre de scénarios stochastiques. La réalisation de cet exercice repose sur trois parties fondamentales qui seront détaillées dans les sections suivantes : les Générateurs de Scénarios Économiques (GSE) pour simuler l'environnement financier, les techniques avancées d'agrégation du passif (\textit{Model Points}) pour réduire le temps des calculs, et enfin l'architecture du modèle de projection ALM qui orchestre les interactions Actif-Passif.
...
Une fois cet environnement financier stochastique modélisé, la seconde brique fondamentale de l'évaluation des sensibilités réside dans la projection du passif. Face à la volumétrie massive des contrats d'assurance vie, une modélisation exhaustive contrat par contrat s'avère opérationnellement inenvisageable sur un grand nombre de scénarios. Il est dès lors impératif de recourir à des techniques de réduction de la dimensionnalité du portefeuille, concept qui introduit la nécessité de regrouper les contrats sous forme de \textit{Model Points}.
...
\begin{itemize}
    ...
    \item \textbf{L'Actif :} Un portefeuille d'investissements soumis aux fluctuations des marchés financiers, dont le rendement permet d'honorer les garanties contractuelles et conditionne la revalorisation de l'épargne sur les supports en euros.
\end{itemize}
...
Si une approche déterministe, reposant sur un unique scénario central, s'avère suffisante pour l'élaboration d'un \textit{Business Plan}, elle demeure structurellement incapable de capturer la valeur temps des options et garanties financières (telles que les Taux Minimum Garantis ou les options de rachat). Le recours à une modélisation stochastique est par conséquent indispensable. Cette méthode consiste à simuler plusieurs milliers de trajectoires économiques (générées via le GSE présenté en section \ref{sec:gse}). L'agrégation de ces simulations permet, d'une part, d'évaluer la moyenne des résultats par la méthode de Monte-Carlo (nécessaire au calcul du \textit{Best Estimate}) et, d'autre part, d'obtenir une distribution empirique complète des résultats. C'est l'analyse des quantiles de cette distribution qui rend alors possible l'estimation de la \textit{Value-at-Risk}\footnote{La \textit{Value-at-Risk} (VaR) est une mesure de risque indiquant la perte potentielle maximale d'un portefeuille sur un horizon temporel donné, pour un niveau de confiance fixé. Dans le référentiel Solvabilité II, le SCR est défini comme la VaR de la variation de l'Actif Net à l'horizon d'un an, avec un seuil de confiance de 99,5\,\%.} (VaR) etfinalement de déterminer le SCR.

\subsection{Algorithmes de projection du passif et valorisation du Best Estimate}
\label{subsec:algo_passif_be}

Contrairement aux modèles traditionnels procédant par itération sur chaque contrat (\og boucle ligne à ligne \fg{}), ce modèle exploite la vectorisation pour projeter simultanément l'ensemble des \textit{Model Points} sur tous les scénarios.
...
\begin{enumerate}
    ...
    \item \textbf{Revalorisation finale et Participation aux Bénéfices (PB) :}
    ...
    \begin{itemize}
        ...
        \item Dotation ou reprise à la Provision pour Participation aux Excédents (PPE) pour lisser les rendements dans le temps. \textcolor{blue}{rq lucas : schéma serait pas mal surtout si tu as des mécanismes pour servir le tx cible}
    \end{itemize}
    ...
    La Provision Mathématique finale du \textit{Model Point} pour l'année $t+1$ est donc :
    \begin{equation}
...
À date, bien que le modèle projette finement les provisions techniques et les actifs, la dynamique des Fonds Propres n'est pas entièrement modélisée. Cette limitation empêche le calcul direct d'un ratio de solvabilité projeté, mais n'impacte pas la qualité des sensibilités sur le Best Estimate et le SCR, qui constituent le cœur de ce mémoire.

Les limites actuelles du modèle résident notamment dans l'absence de modélisation des rachats conjoncturels, une restriction qui impacte fortement la mesure du risque de rachat dans le référentiel Solvabilité II. Intégrer la sensibilité des comportements clients aux évolutions du marché constituerait une piste d'amélioration significative. Dans la même logique, la règle d'attribution de la participation aux bénéfices (PB), pour l'heure rudimentaire, pourrait être enrichie afin de traduire des stratégies de distribution de la richesse plus complexes et réalistes.
