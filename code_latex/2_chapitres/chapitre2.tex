...
Le portefeuille généré représente une Provision Mathématique (PM) totale de $222,09$ milliards d'euros. L'analyse de la répartition de cette richesse révèle une forte asymétrie, caractéristique des portefeuilles d'assurance vie modernes :
\begin{itemize}
    \item La gamme \textbf{Mass Market} ($\text{PM} \leq 50\,k\text{\euro}$) représente la majorité des volumes avec 57,05\% des contrats, mais concentre seulement 4,22\% des encours totaux, avec une PM moyenne par contrat de $16\,366\,\text{\euro}$.
...
\begin{itemize}
    \item \textbf{L'allocation d'actifs :} La part globale investie en Unités de Compte (UC) atteint $44,12\,\%$. Ce ratio, supérieur à la cible de calibration initiale (40\%), s'explique précisément par le poids massif de la Gestion Privée dans les encours (segment structurellement plus investi en UC grâce au modèle Logit).
    \item \textbf{Le Taux Minimum Garanti (TMG) :} Le TMG moyen, pondéré par la PM, s'élève à $1,88\,\%$. Ce niveau est la conséquence directe de la forte ancienneté moyenne du portefeuille (générations antérieures à 2005 bénéficiant de taux techniques généreux).
    \item \textbf{Les frais :} Le taux de frais de gestion sur encours (TFGSE) moyen pondéré ressort à $0,67\,\%$, en parfaite adéquation avec la moyenne du marché.
\end{itemize}
