...
Le portefeuille généré représente une Provision Mathématique (PM) totale de $220,21$ milliards d'euros. L'analyse de la répartition de cette richesse révèle une forte asymétrie, caractéristique des portefeuilles d'assurance vie modernes :
\begin{itemize}
    \item La gamme \textbf{Standard} ($\text{PM} \leq 200\,k\text{\euro}$) représente la majorité des volumes avec 81,78\,\% des contrats, mais concentre seulement 16,52\,\% des encours totaux.
    \item À l'inverse, la \textbf{gestion privée} ($\text{PM} > 500\,k\text{\euro}$) ne concerne que 7,94\,\% des polices mais centralise 68,91\,\% des engagements (PM).
\end{itemize}
...
}, tire la PM moyenne par contrat à un niveau relativement élevé de $220\,212\,\text{\euro}$, positionnant ce portefeuille synthétique sur un profil d'assureur fortement orienté vers la gestion de patrimoine.

\subsubsection*{Profil démographique et ancienneté}
L'âge moyen des assurés s'établit à $53,1$ ans, une valeur cohérente avec la structure des détenteurs d'assurance vie en France. L'ancienneté moyenne des contrats est quant à elle de $19,9$ ans, témoignant d'un portefeuille mature et fortement ancré dans le temps.
...
\begin{itemize}
    \item \textbf{Répartition Euro/UC :} La part globale investie en UC atteint $44,61\,\%$. Ce ratio, légèrement supérieur à la cible de calibration initiale, s'explique précisément par le poids massif de la gamme de gestion privée dans les encours (segment structurellement plus investi en UC grâce au modèle Logit).
    \item \textbf{Le Taux Minimum Garanti (TMG) :} Le TMG moyen, pondéré par la PM, s'élève à $1,90\,\%$. Ce niveau, qui peut paraître élevé dans le contexte de taux de la dernière décennie, est la conséquence directe de la forte ancienneté moyenne du portefeuille (générations antérieures à 2005 bénéficiant de taux techniques généreux).
