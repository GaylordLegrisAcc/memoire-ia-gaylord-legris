...
Au-delà du seul $TMG_t$, la valorisation des engagements en assurance vie dépend fortement des \emph{frais de gestion prélevés sur les encours} et de la \emph{participation aux bénéfices} (PB) effectivement redistribuée aux assurés. Pour capturer ces deux dimensions dans un cadre de modélisation discret, deux paramètres complémentaires sont introduits pour chaque contrat :
\begin{itemize}
...
Dans le modèle, afin de rester cohérent avec cette distribution observée tout en conservant une structure discrète, l'approche retenue s'appuie sur les pratiques tarifaires documentées de contrats patrimoniaux de référence (à l'instar du contrat Cachemire 2 Serie 2 de La Banque Postale \cite{cgs_cachemire}). Trois niveaux de $TFGSE$ sont ainsi définis :
\[
  TFGSE \in \{0{,}60\,\%,\ 0{,}70\,\%,\ 0{,}85\,\%\}.
\]
Ces trois valeurs représentent respectivement :
\begin{itemize}
  \item un profil \og haut de gamme \fg{} ($0{,}60\,\%$), typique des contrats de gestion privée très compétitifs ;
  \item un profil \og moyen \fg{} ($0{,}70\,\%$), proche de la moyenne observée sur les fonds en euros et caractéristique des gammes patrimoniales ;
  \item un profil \og standard \fg{} ($0{,}85\,\%$), représentant des contrats grand public avec une structure de frais plus classique.
\end{itemize}
...
\begin{itemize}
  ...
  \item $90\,\%$ : un comportement \og moyen \fg{} de marché, cohérent avec les constats de l'ACPR sur des marges techniques modérées ;
...
\begin{itemize}
  \item $TMG_t$ est fixé par la table historique de la section précédente, construite à partir des plafonds de taux techniques de l'ACPR ;
  \item $TFGSE$ prend l'une des trois valeurs $\{0{,}60\,\%,\ 0{,}70\,\%,\ 0{,}85\,\%\}$ ;
  \item $TAF$ prend l'une des trois valeurs $\{85\,\%,\ 90\,\%,\ 95\,\%\}$.
\end{itemize}
...
\begin{table}[h!]
  \centering
  \caption{Segmentation des contrats selon la provision mathématique}
  \label{tab:seuils_pm}
  \begin{tabular}{lc}
    \toprule
...
\begin{enumerate}
  \item Traduire mathématiquement la dynamique concurrentielle observée : la gamme \og Standard \fg{} est massivement orientée vers la tarification la moins favorable, tandis que la \og gestion privée \fg{} se voit attribuer majoritairement les conditions optimales.
...
Cette approche par corrélation avec la provision mathématique permet ainsi de générer un portefeuille synthétique dont les caractéristiques microéconomiques (par contrat) s'avèrent cohérentes avec les statistiques macroéconomiques publiées, tout en conservant une structure parfaitement discrète et donc compatible avec l'agrégation ALM.
...
L'objectif est de modéliser la proportion $p_{\text{UC},i} \in (0,1)$ de la Provision Mathématique investie en unités de compte pour chaque contrat $i$. Dans le cadre de cette approche paramétrique, ce modèle repose sur un prédicteur linéaire $\eta_i$ combinant les caractéristiques du contrat et de l'assuré.
...
\begin{itemize}
    ...
    \item \textbf{Le volume de l'épargne ($\log(\text{PM}_i)$)} : Il traduit un \og effet richesse \fg{}. Les encours les plus importants présentent souvent une part d'UC plus élevée, traduisant une recherche de diversification et une meilleure capacité à absorber la volatilité. L'utilisation du logarithme s'avère indispensable pour atténuer la forte asymétrie de la distribution des encours et linéariser cet effet.
\end{itemize}
...
En l'absence de base de données publique croisant l'âge et la part d'UC contrat par contrat, ces points de contrôle ont été définis \textbf{à dire d'expert}. Ils représentent des profils types d'assurés construits pour refléter les standards de la gestion de patrimoine et les politiques commerciales actuelles des assureurs français :
\begin{itemize}
    \item Les jeunes assurés, disposant d'un horizon de placement long, sont orientés vers des profils \og dynamiques \fg{} (forte part d'UC).
    \item À l'inverse, les assurés d'âge avancé s'inscrivent dans une logique de \og sécurisation \fg{} du capital en vue de la transmission, ce qui se traduit par une prédominance écrasante du fonds en euros.
\end{itemize}
...
Bien que cette spécification logistique paramétrique constitue une simplification de la réalité, elle fournit une décomposition macro-économiquement cohérente de la provision mathématique pour chaque assuré simulé. Cette méthode permet ainsi de doter le portefeuille synthétique de profils d'allocation réalistes et différenciés, une condition \textit{sine qua non} pour garantir la pertinence des analyses de sensibilités financières menées dans la suite de ce mémoire.
...
Le segment \og Standard \fg{} ($\text{PM} \leq 50\,k\text{\euro}$) représente la majorité absolue des polices (57\,\%), mais ne pèse que pour 4,2\,\% des encours totaux. À l'inverse, la clientèle de \og Gestion Privée \fg{} ($\text{PM} > 500\,k\text{\euro}$) ne rassemble que 7,9\,\% des assurés mais capte plus de 69\,\% de la Provision Mathématique globale, avec une moyenne avoisinant 1,93 million d'euros par contrat. Ce qui est une concentration extrême pour un portefeuille d'assurance vie classique.
\newpage
...
Ce phénomène s'explique par un \og biais de survivance \fg{} dans la méthode de génération. Les polices ont été créées en dérivant l'âge à la souscription depuis l'âge actuel, sans simuler l'historique d'écoulement du passif. En omettant l'application des lois de mortalité et les lois de rachats sur les deux dernières décennies, le portefeuille \og naît \fg{} directement avec son volume actuel. 
...
Cette carte de chaleur met en évidence la concentration hétérogène des capitaux (les zones les plus denses regroupant la majorité de la Provision Mathématique). C'est précisément dans cet espace bidimensionnel très polarisé que les futurs algorithmes de \textit{clustering} devront concentrer leurs \textit{cluters} pour éviter de lisser le risque.

Sur le plan de la modélisation actif-passif (ALM) et des exigences S2, le couple Âge-Ancienneté est le moteur principal de la dynamique du passif. L'âge pilote à la fois la mortalité et l'allocation Euro/UC. L'ancienneté, quant à elle, régit presque exclusivement la fonction de rachat, en définissant la liquidité fiscale du contrat. Toute erreur de lissage dans ces zones denses déformerait les lois de comportements non-linéaires des assurés, entraînant une mauvaise évaluation du \textit{Best Estimate}.
