...
Concernant la participation aux bénéfices (PB), le cadre réglementaire français impose aux organismes d'assurance vie une redistribution minimale. Celle-ci correspond à $85\,\%$ du solde du compte financier et à $90\,\%$ du solde du compte technique, allouée sous forme de revalorisation immédiate ou de dotation à la provision pour participation aux bénéfices (PPB). Dans la pratique, en raison des enjeux concurrentiels, nous observons que la part des résultats financiers effectivement redistribuée aux assurés excède régulièrement ces minima légaux. Les études de l'ACPR sur la revalorisation des contrats mettent en évidence une contrainte forte sur les marges conservées par les assureurs, le taux de redistribution dépassant fréquemment le seuil réglementaire pour s'aligner sur les attentes du marché \cite{acprTauxBas,acprRevalo2024}.
Cette réalité est modélisée au moyen d'un taux d'affectation des produits financiers, noté $TAF$, de nature discrète, qui représente la part des produits financiers attribuée aux assurés sur un exercice donné. Afin de respecter le seuil légal de distribution, trois niveaux sont retenus :
\[
  TAF \in \{85\,\%,\ 90\,\%,\ 95\,\%\}.
\]
Ces niveaux correspondent à :
\begin{itemize}
  \item $85\,$\% : un comportement correspondant au plancher réglementaire, associé à des contrats peu généreux en termes de participation aux bénéfices ;
...
Pour assurer la cohérence économique du modèle, une dépendance entre le $TFGSE$ et le $TAF$ est introduite au travers de profils de contrats. Concrètement, pour chaque contrat synthétique, il est procédé au tirage d'un profil de frais parmi $\{0{,}50\,\%,\ 0{,}70\,\%,\ 0{,}90\,\%\}$, puis d'un niveau de $TAF$ parmi les trois valeurs $\{85\,\%,\ 90\,\%,\ 95\,\%\}$, au moyen d'une loi discrète conditionnelle. Par exemple, un contrat à faibles frais ($TFGSE = 0{,}50\,$\%) présente une probabilité plus forte d'être associé à un $TAF$ élevé ($90\,\%$--$95\,\%$), tandis qu'un contrat fortement chargé ($TFGSE = 0{,}90\,$\%) se voit plus souvent associé à un $TAF$ dans la partie basse de la grille ($85\,\%$). Ce couplage reproduit qualitativement le fait que les contrats les plus compétitifs en frais sont également ceux qui, historiquement, affichent les meilleurs taux servis et une politique de PB plus favorable aux assurés.
...
\end{itemize}
Ce choix de discrétisation permet de simplifier la génération du portefeuille sur les aspects financiers et assurantiels tout en restant ancré sur des ordres de grandeur observés sur le marché français de l'assurance vie, tels que documentés par France Assureurs et l'ACPR \cite{acprTauxBas,acprRevalo2024,faUC2024}.

\subsubsection*{Corrélation des caractéristiques contractuelles avec la provision mathématique}

Dans la modélisation initiale, les paramètres $(TMG_t, TFGSE, TAF)$ sont tirés indépendamment de l'année de souscription $t$. Cependant, pour refléter fidèlement la structure du portefeuille d'un assureur français, il apparaît pertinent d'introduire une dépendance entre ces caractéristiques contractuelles et la \emph{provision mathématique} du contrat, notée $PM_k$. Celle-ci sert de proxy naturel pour segmenter les contrats par gamme commerciale (Mass Market, Patrimonial, Gestion Privée).

Cette approche s'appuie sur deux observations de marché :
\begin{enumerate}
  \item Les frais de gestion sur encours $TFGSE$ sont généralement \emph{dégressifs} avec l'encours du contrat : les contrats à forte provision mathématique (typiquement supérieurs à 100\,k\euro, voire supérieurs à 500\,k\euro{} pour la Gestion Privée) bénéficient de grilles tarifaires plus favorables \cite{faUC2024}.
  \item Les contrats présentant des encours importants sont historiquement plus concentrés dans les gammes patrimoniales et de Gestion Privée, lesquelles affichent des politiques de participation aux bénéfices (PB) et des taux minimums garantis (TMG) différenciés par rapport aux contrats de la clientèle grand public.
\end{enumerate}
...
\begin{tabular}{lcc}
  ...
  \hline
  Mass Market / Retail & $PM_k \leq 50\,000$\,\euro & Clientèle grand public \\
  Patrimonial & $50\,000$\,\euro $< PM_k \leq 500\,000$\,\euro & Clientèle moyenne / haut de gamme \\
  Gestion Privée & $PM_k > 500\,000$\,\euro & HNWI, family offices \\
  \hline
\end{tabular}
\end{center}

Ces seuils s'inspirent de la segmentation commerciale standard des acteurs français (AXA, CNP Assurances, Generali) et s'avèrent cohérents avec les statistiques agrégées de France Assureurs sur la structure par encours des contrats d'épargne individuelle \cite{faUC2024}.

\subsubsection*{Lois conditionnelles par tranche de provision}

Pour chaque contrat généré, après le tirage de l'année $t$ et de la provision mathématique $PM_k$, les paramètres $(TFGSE, TAF)$ sont définis selon la loi conditionnelle suivante :
...
\begin{tabular}{ll|ccc}
  ...
  & & $(0{,}5\,\%,\ 95\,\%)$ & $(0{,}7\,\%,\ 90\,\%)$ & $(0{,}9\,\%,\ 85\,\%)$ \\
  \hline
  Mass Market & $PM_k \leq 50$\,k\euro & 0,20 & 0,40 & 0,40 \\
  Patrimonial & $50$\,k\euro $< PM_k \leq 500$\,k\euro & 0,40 & 0,40 & 0,20 \\
  Gestion Privée & $PM_k > 500$\,k\euro & 0,60 & 0,30 & 0,10 \\
  \hline
\end{tabular}
\end{table}

Le $TMG$ reste fonction de l'année $t$ uniquement ($TMG_t$ de la table~\ref{tab:TMG_historique}), mais la pondération des générations anciennes (aux $TMG$ élevés) s'avère naturellement plus forte dans les tranches de Gestion Privée, ces contrats ayant eu davantage de temps pour accumuler des provisions élevées.
...
\begin{enumerate}
  ...
  \item Calcul du $TMG$ net : $TMG_t^{net} = \max(TMG_t - TFGSE, 0)$.
\end{enumerate}
...
\begin{itemize}
  ...
  \item \textbf{TMG moyen} : les contrats anciens ($TMG > 2\,\%$) sont naturellement plus présents dans les tranches à forte $PM_k$, ce qui est conforme au taux technique moyen actuel du portefeuille ($0{,}35\,\%$ à $0{,}5\,\%$) rapporté par l'ACPR \cite{acprTauxBas}.
  \item \textbf{Relation frais-rendement} : les contrats à encours importants (GP) présentent à la fois des frais plus bas et un $TAF$ plus élevé, reproduisant le différentiel de performance observé entre les gammes.
\end{itemize}
...
\begin{itemize}
    \item \textbf{L'allocation d'actifs :} La part globale investie en Unités de Compte (UC) atteint $44,44\,\%$. Ce ratio, légèrement supérieur à la cible de calibration initiale, s'explique précisément par le poids massif de la Gestion Privée dans les encours (segment structurellement plus investi en UC grâce au modèle Logit).
