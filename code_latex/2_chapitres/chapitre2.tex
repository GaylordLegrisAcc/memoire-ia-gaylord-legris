...
Le portefeuille généré représente une Provision Mathématique (PM) totale de $222,07$ milliards d'euros. L'analyse de la répartition de cette richesse révèle une forte asymétrie, caractéristique des portefeuilles d'assurance vie modernes :
\begin{itemize}
    \item La gamme \textbf{Mass Market} ($\text{PM} \leq 50\,k\text{\euro}$) représente la majorité des volumes avec 57,05\,\% des contrats, mais concentre seulement 4,22\,\% des encours totaux.
    \item À l'inverse, la \textbf{gestion privée} ($\text{PM} > 500\,k\text{\euro}$) ne concerne que 7,93\,\% des polices mais centralise 69,17\,\% des engagements (PM).
\end{itemize}
...
}, tire la PM moyenne par contrat à un niveau relativement élevé de $222\,071\,\text{\euro}$, positionnant ce portefeuille synthétique sur un profil d'assureur fortement orienté vers la gestion de patrimoine.
...
La calibration des variables financières a porté ses fruits :
\begin{itemize}
    \item \textbf{L'allocation d'actifs :} La part globale investie en Unités de Compte (UC) atteint $44,44\,\%$. Ce ratio, légèrement supérieur à la cible de calibration initiale, s'explique précisément par le poids massif de la gamme de gestion privée dans les encours (segment structurellement plus investi en UC grâce au modèle Logit).
    \item \textbf{Le Taux Minimum Garanti (TMG) :} Le TMG moyen, pondéré par la PM, s'élève à $1,89\,\%$. Ce niveau, qui peut paraître élevé dans le contexte de taux de la dernière décennie, est la conséquence directe de la forte ancienneté moyenne du portefeuille (générations antérieures à 2005 bénéficiant de taux techniques généreux).
    \item \textbf{Les frais :} Le taux de frais de gestion sur encours (TFGSE) moyen pondéré ressort à $0,62\,\%$, en parfaite adéquation avec la moyenne du marché pour des portefeuilles mixtes.
\end{itemize}
