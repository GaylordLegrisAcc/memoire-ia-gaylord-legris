...
Bien que l'algorithme des K-Means s'avère performant pour minimiser la variance au sein des groupes, il repose sur une hypothèse géométrique forte : il tend à construire des partitions sphériques (ou convexes) de tailles comparables. Or, la réalité d'un portefeuille d'assurance vie se caractérise souvent par des densités très hétérogènes et des formes irrégulières. Par exemple, la commercialisation massive d'une génération spécifique de produits sur une courte période crée une concentration locale de polices dont la forme dans l'espace des données est non convexe. \textcolor{blue}{Expliquer le terme convexe à un moment du mémoire}

Pour capter ces niches complexes sans forcer l'homogénéité des volumes ni fixer \textit{a priori} le nombre de \textit{Model Points}, il convient de s'affranchir de la notion de distance pure au profit d'une analyse de la concentration locale des données.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{images/2_chapitres/chapitre3/dbscan.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{images/2_chapitres/chapitre3/dbscan.png}
