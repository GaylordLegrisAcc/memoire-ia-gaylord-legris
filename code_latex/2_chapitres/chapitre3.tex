...
La méthode de \textit{Cash-Flow Matching} constitue l'approche la plus avancée de ce protocole. Elle repose sur un changement de paradigme : au lieu de regrouper les contrats selon leurs caractéristiques statiques, on les regrouper selon leur \textbf{profil de risque dynamique}.
...
\subsubsection{Visualisation et réduction de dimension (PCA / t-SNE)}
Pour valider visuellement la qualité des regroupements avant projection ALM, des techniques de réduction de dimension comme la \textbf{Principal Component Analysis (PCA)} ou le \textbf{t-Distributed Stochastic Neighbor Embedding (t-SNE)} ont été utilisées. Elles permettent de projeter l'espace à 12 dimensions des contrats sur un plan 2D et de vérifier que les clusters formés sont bien compacts et disjoints.
