...
Plus la tranche est large, plus cette variance augmente, ce qui dilue la précision du profil de risque. Le modèle ALM traitant les flux de manière non linéaire (notamment via les probabilités de rachat ou de mortalité), cette perte de dispersion engendre le fameux « biais d'agrégation ». C'est cette limitation fondamentale qui justifie le recours à des algorithmes de partitionnement dynamique (\textit{Machine Learning}), capables de minimiser cette inertie de manière optimale plutôt que de subir une grille prédéfinie.
...
\textcolor{blue}{On isole le CART ici, car il utilise une variable cible, contrairement aux K-Means/HDBSCAN.}
...
\textcolor{blue}{Bilan des méthodes candidates et annonce du protocole de test standardisé (qui fera l'objet du Chapitre 4).}
