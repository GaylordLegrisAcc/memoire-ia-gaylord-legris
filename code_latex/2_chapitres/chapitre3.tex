...
Pour répondre à cette problématique, l'algorithme \textbf{HDBSCAN} (\textit{Hierarchical Density-Based Spatial Clustering of Applications with Noise}) propose une approche hiérarchique permettant de détecter des clusters de densités variables. Son fonctionnement se décompose en cinq étapes séquentielles majeures \cite{hdbscan} :
\begin{enumerate}
    \item La transformation de l'espace métrique (pour isoler le bruit) ;
    \item La construction d'un graphe (arbre couvrant minimum) ;
    \item La formation de la hiérarchie des clusters ;
    \item La condensation de l'arbre hiérarchique ;
    \item L'extraction dynamique des clusters finaux.
\end{enumerate}

Les sous-parties suivantes détaillent la mécanique mathématique de chacune de ces étapes.

\subsubsection{Étape 1 : Transformation de l'espace (la distance d'accessibilité mutuelle)}
...
\begin{figure}[H]
    ...
    \caption{Illustration de la distance d'accessibilité mutuelle \cite{hdbscan} \textcolor{blue}{citer tous les graphes de la section dans le texte pour relier l'explication}}
    \label{fig:mutual_reachability}
\end{figure}

Ayant établi une mesure de distance robuste aux valeurs aberrantes, l'algorithme procède ensuite à la structuration topologique des données.

\subsubsection{Construction de l'arbre couvrant minimum (MST)}
L'algorithme construit ensuite un graphe où chaque contrat est un sommet, relié aux autres par des arêtes pondérées par la distance $d_{mreach}$. Un arbre couvrant minimum (\textit{Minimum Spanning Tree} - MST) est généré pour connecter l'ensemble des points en minimisant le poids total des arêtes.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{images/2_chapitres/chapitre3/hdbscan2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{images/2_chapitres/chapitre3/hdbscan3.png}
...
\begin{itemize}
    \item \textbf{EOM (Excess of Mass) :} C'est la méthode par défaut de l'algorithme, elle sélectionne les clusters présentant la plus grande stabilité en surface dans l'arbre hiérarchique. Cette méthodologie permet d'identifier simultanément des micro-clusters très compacts et des macro-clusters plus étendus, offrant une segmentation optimale et adaptative de la population des assurés sans paramétrage complexe d'un rayon de voisinage.
...
\begin{figure}[H]
    ...
    \caption{Résultat final du clustering HDBSCAN \cite{hdbscan} \textcolor{blue}{citer les graphes et expliquer}}
...
\begin{figure}[H]
    ...
    \caption{Comparaison des approches DBSCAN et HDBSCAN \cite{dbscan_hdbscan} \textcolor{blue}{citer les graphes et expliquer}}
...
\textcolor{blue}{refaire les modélisations en utilisant un peu toutes les méthodes}
