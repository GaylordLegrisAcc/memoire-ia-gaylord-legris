...
\begin{itemize}
    \item \textbf{Variante Refined :} On définit une cible $Y_i = PM_i \times (1 + \alpha \cdot TMG_i)$. Cette pondération force l'arbre à accorder plus d'importance aux contrats qui pèsent lourd financièrement tout en ayant un coût de garantie élevé. C'est une approche visant l'équilibre entre masse et risque.
...
La méthode de \textit{Cash-Flow Matching} constitue l'approche la plus avancée de ce protocole. Elle repose sur un changement de paradigme : au lieu de regrouper les contrats selon leurs caractéristiques statiques, on les regroupe selon leur \textbf{profil de risque dynamique}.

Le principe s'articule en deux phases :
\begin{enumerate}
    \item \textbf{Phase de Projection Proxy :} Chaque contrat individuel $i$ est projeté au sein du moteur métier sous un scénario déterministe central sur un horizon de $T=20$ ans. On extrait de cette projection un vecteur de flux financiers $\mathbf{CF}_i = [f_{i,1}, f_{i,2}, ..., f_{i,T}]$, où $f_{i,t}$ représente la somme des prestations (rachats, décès, termes) et de la marge financière à l'année $t$.
...
L'avantage majeur est la réduction drastique du « biais d'agrégation » puisque le critère de regroupement est précisément l'indicateur que l'on cherche à calculer au final (le Best Estimate). L'inconvénient réside dans le coût computationnel de la pré-projection initiale, qui doit être compensé par le gain de temps lors des lancements ALM ultérieurs.

\subsection{Approche par calibration de lois comportementales a posteriori sur clés d'agrégation financières}

L'approche par calibration \textit{a posteriori} marque une rupture méthodologique avec les techniques de clustering spatial. Là où les méthodes classiques tentent d'ajuster des contrats à des lois de mortalité existantes, cette approche propose d'adapter les lois de sortie pour qu'elles correspondent parfaitement à un regroupement arbitraire de contrats. L'objectif est d'éliminer le biais d'agrégation en cristallisant la dynamique des flux au sein de groupes définis par leur structure de marge.

\subsubsection{Définition des clés d'agrégation et cohérence financière}

Le portefeuille est partitionné selon un triplet de variables financières qui régissent la mécanique de revalorisation. Contrairement aux méthodes démographiques, on regroupe ici les contrats partageant les mêmes caractéristiques de participation aux bénéfices :
\begin{itemize}
    ...
    \item \textbf{Le TAF} : Le taux d'affectation des produits financiers, définissant la part des produits de l'actif revenant contractuellement au passif.
\end{itemize}

Cette segmentation garantit le respect de la relation fondamentale entre le rendement brut et le rendement net du contrat :
\begin{equation}
    TMG_{brut} = \frac{TMG_{net} + TFGSE}{TAF}
\end{equation}

En agrégeant les contrats selon ces trois axes, on s'assure que le \textbf{Model Point Financier (MPF)} résultant possède un seuil de rentabilité brut identique à celui des contrats individuels qui le composent, évitant ainsi toute dilution des garanties ou des marges.

\subsubsection{Protocole de calibration des lois spécifiques}

Une fois les contrats regroupés par identifiants financiers, l'information démographique (âge, sexe) est remplacée par une loi de flux induite. Le processus se déroule en trois étapes :

\begin{enumerate}
    \item \textbf{Projection de référence} : On projette le portefeuille complet (50 000 contrats) en mode passif seul sous un scénario central. On extrait, pour chaque groupe $k$ et chaque année $t$, les prestations de décès ($D_{k,t}$), les rachats ($R_{k,t}$) et l'encours de début de période ($PM_{k,t-1}$).
    
    \item \textbf{Calcul des probabilités induites} : On définit des lois de probabilités « sur-mesure » qui capturent le comportement moyen du groupe :
    \begin{equation}
        q_{k,t}^{\text{décès}} = \frac{\sum D_{i,t}}{\sum PM_{i,t-1}} \quad ; \quad q_{k,t}^{\text{rachat}} = \frac{\sum R_{i,t}}{\sum PM_{i,t-1}}
    \end{equation}
    
    \item \textbf{Substitution des lois} : Lors des projections ALM futures, le modèle n'utilise plus les tables de mortalité classiques. Il appelle, via l'ID du Model Point, le vecteur $[q_{k,1}, \dots, q_{k,T}]$ calibré.
\end{enumerate}

\subsubsection{Analyse de la propriété de « Biais Zéro »}

Cette méthode permet d'obtenir un \textbf{écart rigoureusement nul sur le Best Estimate} en scénario central. Puisque les lois de sortie sont directement issues de la projection exhaustive, le Model Point agrégé reproduit exactement la cinétique des flux originaux. 

Cependant, cette approche rend le modèle « aveugle » à la structure par âge réelle. Pour les analyses de solvabilité, il devient nécessaire de recalculer ces lois induites sous les différents scénarios de choc (choc de mortalité ou choc de rachat) ou d'appliquer des coefficients de déformation aux probabilités $q_{k,t}$ pour refléter la sensibilité démographique sous-jacente.
...
\begin{itemize}
    \item \textbf{Ajout de la dimension Log-PM :} On ajoute le logarithme de la Provision Mathématique comme variable spatiale. Cela force l'algorithme à regrouper des contrats de taille financière similaire, évitant qu'un « gros » contrat ne soit noyé dans une masse de « petits » contrats.
