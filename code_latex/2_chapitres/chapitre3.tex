...
de la génération, de l'âge et du sexe ; les rachats sont corrélés à l'âge de l'assuré et à l'ancienneté du contrat (fiscalité) ; la revalorisation dépend du Taux Minimum Garanti (TMG).
...
Afin de rendre l'algorithme plus robuste aux points aberrants (bruit), HDBSCAN ne travaille pas directement sur la distance euclidienne brute, mais définit une nouvelle métrique : la \og distance d'accessibilité mutuelle \fg{}.
...
\subsection{Calibration de lois \textit{a posteriori} (Méthode \og Biais Zéro \fg{} et clés financières)}
...
Pour que la méthode du \og Biais Zéro \fg{} fonctionne intégralement, il est nécessaire de garantir une stricte fidélité dans la projection des flux de revalorisation futurs. 

Contrairement aux flux de rachats et de décès qui sont linéaires (et peuvent donc être parfaitement lissés par des taux induits), la mécanique de Participation aux Bénéfices (PB) est intrinsèquement non linéaire, notamment en raison de la composante optionnelle des garanties financières ($\max(0, Tx_{\text{servi}} - TMG)$). Il est par conséquent mathématiquement impossible d'agréger des contrats aux caractéristiques financières hétérogènes sans créer de biais sur la revalorisation.
...
Cette segmentation garantit que le \textbf{Model Point Financier (MPF)} possède un seuil de rentabilité brut rigoureusement identique à celui des contrats individuels qui le composent :
\begin{equation}
...
Une fois les contrats regroupés selon leurs clés financières (formant ainsi les groupes $C_k$), il convient d'annuler l'erreur d'agrégation sur la dimension biométrique et comportementale. Pour estimer les probabilités sur-mesure, il est impératif de faire tourner le modèle ALM une première fois en vision granulaire (contrat par contrat) sur un scénario de base, en mode \og passif seul \fg{} (sans interaction avec l'actif ni participation aux bénéfices dynamique). 
...
Sur le plan opérationnel, l'implémentation de cette méthode exige une adaptation du cœur du modèle ALM. Le module de vieillissement du passif ne doit plus faire évoluer l'âge ou l'ancienneté pour lire une table réglementaire externe, mais est reprogrammé pour lire et appliquer directement les vecteurs de probabilités de sortie ($q_{k,t}$) pré-calculés, rattachés à chaque \textit{Model Point}. Cette approche, en capturant l'intégralité de la cinétique des flux, permet d'atteindre le \og Biais Zéro \fg{} opérationnel sur l'ensemble des projections.
