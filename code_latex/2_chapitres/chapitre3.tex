...
\section{Présentation des Méthodes d'Agrégation candidates}
    ...
    L'application directe de DBSCAN aux données brutes d'un portefeuille d'assurance se heurte à deux obstacles majeurs nécessitant des adaptations spécifiques implémentées dans notre protocole : Le problème de la discrétisation et la gestion du bruit.

    Les variables de gestion (Âge, Ancienneté) sont traditionnellement stockées sous forme d'entiers. Cette structure crée une grille artificielle où des milliers de contrats se superposent exactement sur les mêmes coordonnées $(x,y)$. Cela fausse le calcul de densité locale : un point isolé sur la grille peut artificiellement paraître très dense simplement parce qu'il superpose plusieurs contrats identiques.
    
    Pour y remédier, nous appliquons une technique de \textbf{Dithering} (bruitage uniforme). Avant l'étape de clustering, une perturbation aléatoire $u_i$ est ajoutée aux variables temporelles de chaque contrat $x_i$ :
    $ \tilde{x}_{i, d} = x_{i, d} + u_{i, d} \quad \text{avec} \quad u_{i, d} \sim \mathcal{U}([-0.5, 0.5]) $
    Cette transformation permet de \og fluidifier \fg{} l'espace et de révéler la véritable topologie de densité des contrats, sans altérer les propriétés statistiques globales du portefeuille (l'espérance de la perturbation étant nulle).

    Contrairement à une analyse de données classique exploratoire où le bruit peut être écarté, dans un modèle épargne, la complétude des engagements est une contrainte absolue : $\sum PM_{MP} = \sum PM_{contrats}$. Les contrats classés comme \og bruit \fg{} par DBSCAN (zones de faible densité ou points isolés) ne peuvent être ignorés.
   ...
   Il a donc fallu implémenter une étape de post-traitement systématique : une réallocation via un algorithme des \textbf{$k$-plus proches voisins ($k$-NN)} avec $k=1$. Chaque contrat identifié comme bruit $x_{noise}$ est réaffecté au cluster validé $C_j$ le plus proche :
    ...
    \subsubsection{L'évolution vers HDBSCAN}
    Une limite intrinsèque de DBSCAN est l'utilisation d'un seuil de densité global ($\varepsilon$) unique pour tout l'espace. Or, un portefeuille d'assurance est hétérogène : il contient des zones très denses (cœur de cible commercial récent) et des zones diffuses (anciens produits en \textit{run-off}). Un $\varepsilon$ trop petit fragmente les zones diffuses en bruit, tandis qu'un $\varepsilon$ trop grand fusionne des clusters distincts dans les zones denses.

    Pour pallier cette rigidité, l'algorithme \textbf{HDBSCAN} (\textit{Hierarchical DBSCAN}) a été créé. Au lieu de fixer un rayon $\varepsilon$, l'algorithme construit une hiérarchie de clusters pour toutes les valeurs possibles de $\varepsilon$ (ou plutôt $\lambda = 1/\varepsilon$).
    
    La sélection des clusters optimaux ne se fait plus par une coupure horizontale de l'arbre (comme dans les méthodes hiérarchiques classiques), mais en optimisant la stabilité des clusters via la mesure \textit{Excess of Mass} (eom). Pour chaque cluster potentiel $C$, on calcule une stabilité $\mathcal{S}(C)$ basée sur la persistance des points dans le cluster à travers les différents niveaux de densité $\lambda$ :
    $ \mathcal{S}(C) = \sum_{x \in C} (\lambda_{death}(x) - \lambda_{birth}(C)) $
    Où $\lambda_{death}(x)$ est la valeur de densité à laquelle le point $x$ quitte le cluster (devient du bruit ou change de branche), et $\lambda_{birth}(C)$ est la densité à laquelle le cluster $C$ apparaît. L'algorithme sélectionne ensuite l'ensemble de clusters disjoints qui maximise la somme des stabilités.
    
    Cette approche permet d'identifier simultanément des micro-clusters très denses et des macro-clusters plus étendus, offrant une segmentation beaucoup plus fine et adaptative de la population des assurés, particulièrement adaptée à la diversité des portefeuilles vie.

    \figure[h!]
        \centering
        \includegraphics[width=0.8\textwidth]{images/2_chapitres/chapitre4/dbscan_hdbscan.jpg}
        \caption{Différence entre DBSCAN et HDBSCAN\cite{dbscan_hdbscan}}
        \label{fig:hdbscan}
    \endfigure
