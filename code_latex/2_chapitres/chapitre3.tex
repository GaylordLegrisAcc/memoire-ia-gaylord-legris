...
\section{Présentation des Méthodes d'Agrégation candidates}
   ...
   Il a donc fallu implémenter une étape de post-traitement systématique : une réallocation via un algorithme des \textbf{$k$-plus proches voisins ($k$-NN)} avec $k=1$. Chaque contrat identifié comme bruit $x_{noise}$ est réaffecté au cluster validé $C_j$ le plus proche :
    ...
    \figure[h!]
        \centering
        \includegraphics[width=0.7\textwidth]{images/2_chapitres/chapitre4/dbscan.png}
        \caption{Explication des reclassement du bruit (TODO : A faire)}
    ...
    Bien que l'algorithme DBSCAN permette de s'affranchir de la contrainte de sphéricité des clusters propre aux K-Means, il conserve une limite majeure : l'utilisation d'un seuil de densité global ($\varepsilon$) unique pour tout l'espace. Dans un portefeuille d'assurance vie, la densité des données est par nature hétérogène. Certaines zones, correspondant aux produits phares récemment commercialisés, présentent une très forte concentration de contrats, tandis que d'autres, regroupant des générations anciennes ou des produits de niche, sont beaucoup plus diffuses. L'application d'un $\varepsilon$ unique conduit inévitablement à un compromis insatisfaisant : un seuil strict fragmente les zones diffuses en bruit, tandis qu'un seuil lâche fusionne des clusters distincts dans les zones denses.

    Pour répondre à cette problématique, l'algorithme \textbf{HDBSCAN} (\textit{Hierarchical Density-Based Spatial Clustering of Applications with Noise}) propose une approche hiérarchique permettant de détecter des clusters de densités variables. Son fonctionnement peut être décomposé en cinq étapes clés \cite{hdbscan} :

    \begin{enumerate}
        \item \textbf{Transformation de l'espace (Mutual Reachability Distance) :} 
        ...
        Où $\text{core}_k(x)$ est la distance du point $x$ à son $k$-ième voisin le plus proche. Cette transformation a pour effet d'éloigner les points situés dans des zones de faible densité (dont la distance de cœur est grande), tout en conservant les distances intactes dans les zones denses. Cela permet de « diluer » le bruit.
    
        \item \textbf{Construction de l'Arbre Couvrant Minimum (MST) :} 
        L'algorithme construit ensuite un graphe où chaque contrat est un sommet, relié aux autres par des arêtes pondérées par la distance $d_{mreach}$. Un Arbre Couvrant Minimum (Minimum Spanning Tree) est généré pour connecter l'ensemble des points en minimisant le poids total des arêtes.
    
        \item \textbf{Construction de la hiérarchie des clusters :} 
